...
class _FollowersScreenState extends State<FollowersScreen> {
  List<Users> _userFollowers = [];
  List<Users> _userFollowing = [];
  ...
  Future _setupFollowers() async {
    List<String> userFollowersIds =
        await DataBase.getUserFollowersIds(widget.user.id);

    List<Users> userFollowers = [];
    List<bool> userFollowersState = [];
    for (String userId in userFollowersIds) {
      Users user = await DataBase.getUserWithId(userId);
  ...
  Future _setupFollowing() async {
    List<String> userFollowingIds =
        await DataBase.getUserFollowingIds(widget.user.id);
  ...
  _removeFollowerDialog(Users user, int index) {
    showDialog(
        context: context,
  ...
  _buildFollowerButton(Users user, int index) {
  ...
  _goToUserProfile(BuildContext context, Users user) {
    showProfile(context, profileId: user.id,);
  }

  _buildFollower(Users user, int index) {
    return ListTile(
  ...
  _buildFollowingButton(Users user, int index) {
    return FlatButton(
  ...
  _buildFollowing(Users user, int index) {
    return ListTile(
      leading: CircleAvatar(
        radius: 25.0,
        backgroundColor: Colors.grey,
                ...
                children: [
                  RefreshIndicator(
                    ...
                    child: ListView.builder(
                      ...
                      itemBuilder: (BuildContext context, int index) {
                        Users follower = _userFollowers[index];
                  ...
                  RefreshIndicator(
                    ...
                    child: ListView.builder(
                      ...
                      itemBuilder: (BuildContext context, int index) {
                        Users follower = _userFollowing[index];
